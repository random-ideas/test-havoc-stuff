...
#Setup remote ssh forwarding
RUN apt-get update && apt-get install openssh-server -y
RUN echo "#Adding new lines to ssh config " >> /etc/ssh/sshd_config
RUN echo "X11DisplayOffset 10" >> /etc/ssh/sshd_config
RUN echo "X11UseLocalhost no" >> /etc/ssh/sshd_config
RUN echo "PermitRootLogin yes" >> /etc/ssh/sshd_config
RUN echo "Port 2222" >> /etc/ssh/sshd_config

RUN useradd -m -p $(openssl passwd -1 <PUT IN WHATEVER YOUR WANT HERE>) -s /bin/bash tester 
#RUN touch /home/tester/.Xauthority
#RUN service ssh start
ENTRYPOINT service ssh start && /opt/Havoc/havoc server --profile ./profiles/havoc.yaotl -v --debug
